"""
s9--s10 队友只在被明确指派时才行动，领导给每个队友写prompt，任务看板上10个未认领的任务得手动分配，这扩展不了

真正的自治：队友字节扫描任务看板，认领没人做的任务，做完再找下一个
一个细节: Context Compact (s6) 后 Agent 可能忘了自己是谁。身份重注入解决这个问题。
...
+---------+
    |
    +----> check inbox --> messages ? --> resume work
    |
...
mesages = [identity_block,...,remaining,...]
...
class MessageBus:
    ...
    # 读取相应的jsonl文件，然后将该读取内容返回，同时清空该jsonl文件内容
