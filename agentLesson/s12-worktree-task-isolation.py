"""
到 s11, Agent 已经能自主认领和完成任务。但所有任务共享一个目录。两个 Agent 同时重构不同模块 -- A 改 config.py, B 也改 config.py,
未提交的改动互相污染, 谁也没法干净回滚。
...
class TaskManager:
    ...
    # 根据task编号判断json文件是否存在
    def exists(self, task_id: int) -> bool:
        return self._path(task_id).exists()

    # 根据task编号来更新相应的json文件
...
class WorktreeManager:
    ...
    def _is_git_repo(self) -> bool:
        try:
            # 用来判断当前目录是否位于一个 Git 仓库的工作树（Work Tree）内部
            r = subprocess.run(
    ...
    def create(self, name: str, task_id: int = None, base_ref: str = "HEAD") -> str:
        ...
        try:
            # git worktree add -b <新分支名> <目录路径> <起点(可选)>
            # 用来创建新的工作区的同时，新建并检出一个分支
